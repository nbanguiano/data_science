...
% [J, grad] = COFICOSTFUNC(params, Y, R, num_users, num_movies, num_features, lambda) returns the cost and gradient for the collaborative filtering problem.
...
prediction = X * Theta';
error = prediction - Y;
mappedRatings = (error.^2).*R;

% Regularization
% users = sum(R, 1) != 0;
X_reg = (lambda * sum(X(:).^2)) / 2;

% movies = sum(R, 2) != 0;
Theta_reg = (lambda * sum(Theta(:).^2)) / 2;

% UnReg Cost
unRegCost = sum(sum(mappedRatings)) / 2;

% Regularized Cost
J = unRegCost + X_reg + Theta_reg;
